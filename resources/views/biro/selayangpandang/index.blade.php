...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Selayang Pandang');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/biro/tentang/selayang-pandang-store', 'Tambah',
                    'Selayang Pandang',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/biro/tentang/selayang-pandang-update/' + uuid,
                    'Update',
                    'Selayang Pandang', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            ...
            $(".title_side_form").html(`Update Selayang Pandang`);
            $(".text-danger").html("");
            let url = '/biro/tentang/selayang-pandang-show/' + $(this).attr('data-uuid');
            $.ajax({
                url: url,
                method: "GET",
                success: function(res) {
                    if (res.success == true) {
                        // Cek apakah TinyMCE sudah diinisialisasi
                        const editor = tinymce.get('deskripsi');

                        if (editor) {
                            // Jika sudah diinisialisasi (biasanya saat mode "edit"), langsung set konten
                            editor.setContent(res.data.deskripsi || '');
                        } else {
                            // Jika belum diinisialisasi (biasanya saat pertama buka atau "tambah"), inisialisasi dan set konten setelah siap
                            tinymce.init({
                                selector: "#deskripsi",
                                height: 480,
                        ...
                        $.each(res.data, function(x, y) {
                            ...
                            const $selectArray = $("select[name='" + x + "[]']");
                            const $textarea = $("textarea[name='" + x + "']");
                            ...
                            if ($input.is(":radio")) {
                                ...
                                $('#logoInfoContainer').html(
                                    `<img id="img-foto" src="/public/selayang-pandang/${y}" style="max-width:100%;">`
                                );
                            } else if ($selectArray.length) {
                                $selectArray.val(y).trigger("change");
                            } else {
                                $input.val(y);
                                $select.val(y).trigger("change");
                                $textarea.val(y);
                            }
                        });
                    }
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/biro/tentang/selayang-pandang-delete/' + $(this).attr('data-uuid');
                            ...
                            result =
                                ...
                                <a class="d-block overlay fancybox" data-fancybox="lightbox-group" href="{{ asset('/public/selayang-pandang/${data}') }}">
                                    ...
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-100px"
                                        style="background-image:url('/public/selayang-pandang/${data}')">
                                    </div>
                                    <!--end::Image-->
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Selayang Pandang" class="btn btn-danger button-delete btn-icon btn-sm ${full.role == 'biro' ? 'disabled' : ''}">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        //     e.preventDefault();
        //     window.open(`/biro/tentang/selayang-pandang-export`, "_blank");
        // });
    </script>
@endsection
