...
@section('content')
    ...
    <div
        ...
        <div class="text-center text-md-start mb-5 mb-md-0">
            <h2 class="mb-1 mb-text-h2 mb-text-color-primary mb-brand-primary-color">Gedung & Fasilitas</h2>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Fasilitas');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/biro/tentang/fasilitas-store', 'Tambah',
                    'Fasilitas',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/biro/tentang/fasilitas-update/' + uuid,
                    'Update',
                    'Fasilitas', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            ...
            $(".text-danger").html("");
            let url = '/biro/tentang/fasilitas-show/' + $(this).attr('data-uuid');
            $.ajax({
                url: url,
                method: "GET",
                    ...
                    if (res.success == true) {
                        ...
                        $.each(res.data, function(x, y) {
                            ...
                            if ($("input[name='" + x + "']").is(":radio")) {
                                ...
                                $('#logoInfoContainer').html(
                                    `<img id="img-foto" src="/public/fasilitas/${y}" style="max-width:100%;">`
                                );
                            } else {
                                $("input[name='" + x + "']").val(y);
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/biro/tentang/fasilitas-delete/' + $(this).attr('data-uuid');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/biro/tentang/fasilitas-get',
                columns: [{
                    data: null,
                    ...
                    data: 'nama_fasilitas',
                    className: 'mb-kolom-text text-center',
                }, {
                    data: 'deskripsi',
                    className: 'mb-kolom-text text-center',
                }, {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        result =
                            `
                                <!--begin::Overlay-->
                                <a class="d-block overlay fancybox" data-fancybox="lightbox-group" href="{{ asset('/public/fasilitas/${data}') }}">
                                    <!--begin::Image-->
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-100px"
                                        style="background-image:url('/public/fasilitas/${data}')">
                                    </div>
                                    <!--end::Image-->
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Fasilitas" class="btn btn-danger button-delete btn-icon btn-sm ${full.role == 'biro' ? 'disabled' : ''}">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        //     e.preventDefault();
        //     window.open(`/biro/tentang/fasilitas-export`, "_blank");
        // });
    </script>
@endsection
