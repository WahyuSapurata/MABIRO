...
@section('content')
    ...
    <div
        ...
        <div class="text-center text-md-start mb-5 mb-md-0">
            <h2 class="mb-1 mb-text-h2 mb-text-color-primary mb-brand-primary-color">Laporan & Keluhan Warga</h2>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Laporan & Keluhan');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/biro/keluhan-store', 'Tambah',
                    'Laporan & Keluhan',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/biro/keluhan-update/' + uuid,
                    'Update',
                    'Laporan & Keluhan', 'POST');
            }
        });

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/biro/keluhan-show/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Laporan & Keluhan', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/biro/keluhan-delete/' + $(this).attr('data-uuid');
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Laporan & Keluhan" class="btn btn-danger button-delete btn-icon btn-sm ${full.role == 'biro' ? 'disabled' : ''}">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        //     e.preventDefault();
        //     window.open(`/biro/keluhan-export`, "_blank");
        // });
    </script>
@endsection
