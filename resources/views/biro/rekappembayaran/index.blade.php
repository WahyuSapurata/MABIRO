...
@section('content')
    ...
    <div
        ...
        <div class="text-center text-md-start mb-5 mb-md-0">
            <h2 class="mb-1 mb-text-h2 mb-text-color-primary mb-brand-primary-color">Rekap Tagihan Warga</h2>
            <p class="mb-0 mb-text-p18 mb-text-color-secondary">Asrama Mahasiswa Balikpapan KPMB Makassar</p>
        </div>

        <!-- Kanan: Tombol -->
        <div class="text-center text-md-end">
            <button class="btn mb-btn-tambah-data btn-sm " id="buat-invoice"><i class="fa fa-plus-circle" style="color:#ffffff"
                    aria-hidden="true"></i> Terbitkan Tagihan</button>
        </div>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#buat-invoice', function() {
            $.ajax({
                url: '/biro/keuangan/buat-invoice',
                method: "GET",
                success: function(res) {
                    if (res.success == true) {
                        swal
                            .fire({
                                text: "Berhasil menerbitkan tagihan",
                                icon: "success",
                        ...
                        swal
                            .fire({
                                text: "Tagihan sudah dibuat untuk bulan ini",
                                icon: "warning",
                                showConfirmButton: false,
                                timer: 1500,
                            });
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'update') {
                ...
                control.submitFormMultipartData('/biro/keuangan/rekap-pembayaran-update/' + uuid,
                    'Update',
                    'Rekap Pembayaran', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            ...
            $(".title_side_form").html(`Update Rekap Pembayaran`);
            $(".text-danger").html("");
            let url = '/biro/keuangan/rekap-pembayaran-show/' + $(this).attr('data-uuid');
            $.ajax({
                url: url,
                method: "GET",
                success: function(res) {
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/biro/keuangan/rekap-pembayaran-delete/' + $(this).attr('data-uuid');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/biro/keuangan/rekap-pembayaran-get',
                columns: [{
                    data: null,
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        if (data) {
                            result =
                                `
                                <!--begin::Overlay-->
                                <a class="d-block overlay fancybox" data-fancybox="lightbox-group" href="{{ asset('/public/bukti/${data}') }}">
                                    <!--begin::Image-->
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded h-50px w-50px"
                                        style="background-image:url('/public/bukti/${data}')">
                                    </div>
                                    <!--end::Image-->
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Rekap Pembayaran" class="btn btn-danger button-delete btn-icon btn-sm ${full.role == 'biro' ? 'disabled' : ''}">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        //     e.preventDefault();
        //     window.open(`/biro/keuangan/rekap-pembayaran-export`, "_blank");
        // });
    </script>
@endsection
