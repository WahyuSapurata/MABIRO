                                    ...
                                    {{-- <tfoot id="tfoot" class="bg-info rounded d-none">
                                        <tr class="fw-bolder fs-6 text-white">
                                            ...
                                            <td style="text-align: left !important; font-weight: bolder" id="total-iuran">Rp
                                                0</td>
                                            <td style="text-align: right !important; font-weight: bolder"
                                                colspan="{{ $materTagihan->count() - 2 }}" id="total-data">Rp
                                                0</td>
                                            <td style="text-align: left !important; font-weight: bolder" id="total-beban">Rp
                                                0</td>
                                            <td style="text-align: right !important; font-weight: bolder"
                                                id="total-listrik">Rp 0</td>
                                            <td style="text-align: right !important; font-weight: bolder" id="total">Rp
                                                0</td>
                                            <td></td>
                                        </tr>
                                    </tfoot> --}}
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Tagihan');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/biro/keuangan/tagihan-store', 'Tambah',
                    'Tagihan',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/biro/keuangan/tagihan-update/' + uuid,
                    'Update',
                    'Tagihan', 'POST',
                    function success() {
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            $(".title_side_form").html(`Update Tagihan`);
            $(".text-danger").html("");
            let url = '/biro/keuangan/tagihan-show/' + $(this).attr('data-uuid');
            $.ajax({
                url: url,
                method: "GET",
                success: function(res) {
                    if (res.success == true) {
                        ...
                        // Loop setiap properti di res.data
                        ...
                        // Tangani checkbox uuid_master_tagihan[] secara khusus
                        let selectedTagihan = data.uuid_master_tagihan;
                        if (Array.isArray(selectedTagihan)) {
                            $("input[name='uuid_master_tagihan[]']").each(function() {
                                let val = $(this).val();
                                if (selectedTagihan.includes(val)) {
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/biro/keuangan/tagihan-delete/' + $(this).attr('data-uuid');
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Tagihan" class="btn btn-danger button-delete btn-icon btn-sm ${full.role == 'biro' ? 'disabled' : ''}">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        $(function() {
            dataPenghuni('#uuid_penghuni');
            initDatatable();
        });

        // $('#export-excel').click(function(e) {
        //     e.preventDefault();
        //     window.open(`/biro/keuangan/tagihan-export`, "_blank");
        // });
    </script>
@endsection
